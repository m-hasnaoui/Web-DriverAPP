...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    if (isset($_GET['startDate']) &&isset($_GET['endDate'])) {
        $query = "SELECT u.username,caisse.* FROM caisse  join users u on caisse.idUser=u.idUser WHERE date_caisse BETWEEN :startDate AND :endDate  AND idSolde=2; ";
        $params = array(':startDate' => $_GET['startDate'],':endDate' => $_GET['endDate']);
        $result = metodGet($query, $params);
        echo json_encode($result->fetchAll(PDO::FETCH_ASSOC));
    //     else {
    //     $query = "SELECT * FROM caisse where idSolde=2";
...
if ($_POST['METHOD'] == 'POST') {
    ...
    $query = "INSERT INTO caisse(date_caisse, designation, num_bl, montant,sens, idSolde,idUser) 
              VALUES (:date_caisse, :designation, :num_bl, :montant, :sens, 2,:idUser);";
    if ($sens == 1) {
        $query .= " UPDATE solde_caisse SET solde = (solde + :montant) WHERE idSolde = 2;";
    } elseif ($sens == -1) {
        $query .= " UPDATE solde_caisse SET solde = (solde - :montant) WHERE idSolde = 2;";
    } else {
...
if ($_POST['METHOD'] == 'PUT') {
    ...
    $query = "UPDATE caisse SET date_caisse = :date_caisse, designation = :designation, 
              num_bl = :num_bl, montant = :montant,  sens = :sens, idSolde = 2 
