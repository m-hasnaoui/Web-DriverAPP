...
if ($_POST['METHOD'] == 'PUT') {
    ...
    $query = "UPDATE livraison SET idM=:idM, blNumber=:blNumber, fctNumber=:fctNumber, nbr_camion_onee=:nbr_camion_onee, prix_fct=:prix_fct, brut=:brut, tare=:tare, net=:net, comentaire=:comentaire, observation=:observation, deposit = :deposit ,valide=:valide WHERE idL=:idL"; 
    $params = array(':idL' => $idL,':idM' => $idM, ':blNumber' => $blNumber, ':fctNumber' => $fctNumber, ':nbr_camion_onee' => $nbr_camion_onee, ':prix_fct' => $prix_fct, ':brut' => $brut, ':tare' => $tare, ':net' => $net, ':comentaire' => $comentaire, ':observation' => $observation,':deposit' => $deposit,':valide'=>$valide);
